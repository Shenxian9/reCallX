...
- bookmarks/likes 先反复 `scrollTo` 当前底部并等待虚拟列表扩展；底部位置和高度连续
  3 轮稳定后开始记录。
- bookmarks/likes 记录阶段每轮向上滚动约视口高度的 75%，到达顶部且连续 8 轮
  没有新增记录时停止。
- follows 每轮向下滚动约视口高度的 75%，到达底部且连续 8 轮没有新增记录时停止。
- 每次滚动后等待 2 秒，每个阶段最多 80 轮。
