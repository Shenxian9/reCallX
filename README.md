...
一键更新只读取页面渲染出的链接，不调用 X 内部 API。每个页面最多执行 20 次有界滚动，
连续多次没有新页面高度后会提前停止，因此它不是无限滚动。

> **覆盖提醒：** 一键更新会用本次扫描结果整体替换对应的 likes、bookmarks、follows。
> 如果页面未完整加载、账号无权限查看某分类或有限滚动未覆盖全部历史，本地旧记录可能被
> 覆盖或删除。脚本会在启动前要求用户确认。
